...
class SubwayMapApp {
	...
	async fetchData() {
		...
		try {
			...
			this.allTrainData = rawData.filter((feed) => feed.data && !feed.error).flatMap((feed) => feed.data);
	...
	updateTrainList() {
		...
		this.allTrainData.forEach((train) => {
			const label = (train.displayRoute || train.routeId || "").toString();
			if (!label) return;
			if (!trainsByRoute[label]) {
				trainsByRoute[label] = [];
			}
			trainsByRoute[label].push(train);
		});

		Object.entries(trainsByRoute)
			.sort(([routeA], [routeB]) => routeA.localeCompare(routeB, undefined, { numeric: true }))
	...
	getLineColor(line) {
		...
		// Unknown routes (e.g., LIRR) will use a neutral color
